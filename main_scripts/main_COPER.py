...
from comparison_DL.COPER.src.coper_model import COPER
import comparison_DL.COPER.src.utils as utils_coper
...
def fit(model, criterion, learning_rate,\
  ...
  for epoch in range(0, n_epochs):   
    ...
    for batch_data in tqdm(train_dataloader):
      ...
      time_steps = torch.tensor(np.array([i for i in range(series.shape[1])])/float(series.shape[1]-1)).to(device)
      time_steps = time_steps.unsqueeze(0)
      remove_t, objd_t = time_steps, time_steps
      y_pred = model(series, time_steps, objd_t, remove_t)
...
if __name__ == '__main__':    
  ...
  args = utils_coper.init_args()

  model = COPER(args, n_labels=output_size, input_dim=input_size, num_latents=48, latent_dim=8, 
                rec_layers=3, units=100, nonlinear=nn.Tanh, cont_in=True, cont_out=True, 
                emb_dim=32, device=device).to(device)
