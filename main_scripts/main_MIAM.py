...
from comparison_DL.MIAM.MIAM.models import Multi_Duration_Pipeline_Residual
...
model = Multi_Duration_Pipeline_Residual(input_dim=33, d_model=64, d_ff=128, 
                                         num_stack=2, num_heads=8, max_length=series_length_limit, n_iter=1).to(device)
  ...
  for epoch in range(0, n_epochs):   
    ...
    for batch_data in tqdm(train_dataloader):
      ...
      if series_length < series_length_limit:
         ...
         mask = torch.cat((mask, torch.zeros((mask.size(0), series_length_limit - series_length, mask.size(2))).cuda()), dim=1)
         delta = torch.cat((delta, torch.zeros((delta.size(0), series_length_limit - series_length, delta.size(2))).cuda()), dim=1)
      else:
         series = series[:, :series_length_limit, :]
         time = time[:, :series_length_limit]
         mask = mask[:, :series_length_limit, :]
         delta = delta[:, :series_length_limit, :]
      attn_mask = delta.data.eq(0)[:, :, 0]
      attn_mask[:, 0] = 0
      y_pred, _ = model(series, mask, time, delta, attn_mask)
